...
if (opts.help || pathCount === 0) usage();
...
function getModuleInfo(fullPath) {
  function commonJS(modulePath, packageId) {
    ...
    var moduleId = (packageId !== null) ? packageId + ":" + modulePart : modulePart;
...
function processPath(p) {
  ...
  var id = idFor(st);
  if (id in idsSeen) return; // avoid loops
...
function processMany() {
  ...
  for (i = 0; i <= pcm1; i++) {
    ...
    fileinfo[i] = {path: f, lines: l, data: data, start: linecount + 1, end: linecount + l};
    linecount += l;
  }
